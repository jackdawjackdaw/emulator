...
##
## eg for some set of design points Design.points and realisatoins of Y_m Training.values
...
##
## To estimate the correct values for these hyperparameters we use the R function callEstimate.
##
## The set of estimated hyperparameters Theta, along with Y_t and D fully specify our emulator, we can now
...
estimate.model <- function(model){
  ...
  ##
  ## note that model must contain xmodel and training slots which contain the model design and the evaluations of the
  ## model at these design points
  ##
  ## IMPORTANT: models with multidimensional parameter spaces xmodel should be a matrix with nparams columns, each row of the matrix
  ## specifies a location in the nparams-d design space.
  ##
...
## this is hopefully enough to make predictions from a 1d model, for a higher dimensional parameter space the only things that should change are:
...
## you'll need to figure out a way to plot a n-dim model (plot.model won't gracefully expand), the R image and contour plots are pretty good.
##
## enjoy...
